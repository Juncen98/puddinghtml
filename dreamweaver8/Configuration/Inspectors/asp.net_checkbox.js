...
var helpDoc = MM.HELP_piASPNETCheckBox;
...
function initializeUI() {
  TEXT_ID = dwscripts.findDOMObject("cboxID");
  ...
  // Initialize text fields to empty string and checkboxes to default state
  // so that previous values don't stick.
  TEXT_ID.value = "";
  TEXT_TEXT.value = "";
    ...
    switch (attrib){
      ...
      case "dynamicChecked":
        ...
        if (sbObj){
          ...
          dw.popupServerBehavior("Dynamic Checkbox.htm");
        }
        break;
...
function getSBObj(theTagObj){
  ...
  var sbObj = null;
  var theName = theTagObj.getAttribute("id");
  for (var i=0; i < sbObjs.length; i++){
    if (sbObjs[i].getTitle() == MM.LABEL_DynCheckboxTitle.replace(/@@theName@@/, theName)){
      sbObj = sbObjs[i];
      break;
    }
  }
