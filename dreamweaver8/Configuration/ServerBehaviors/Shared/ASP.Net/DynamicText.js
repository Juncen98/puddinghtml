...
{
  var sbArray = dwscripts.findSBs(MM.LABEL_DynamicTextTitle + " (@@source@@.@@binding@@)");
...
function applyServerBehavior(sbObj)
{
  ...
  if (expression)
  {
    if (sbObj)
    {
	    if (priorSrc && nodeSeg)
      {
        dwscripts.queueNodeEdit(expression, nodeSeg.node, nodeSeg.matchRangeMin, nodeSeg.matchRangeMax);
    ...
    else
    {
      ...
      dwscripts.queueDocEdit(expression, editType);
      dwscripts.applyDocEdits();
    }
  }
