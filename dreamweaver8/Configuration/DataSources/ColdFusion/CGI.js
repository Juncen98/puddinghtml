...
function addDynamicSource()
{
  MM.retVal = "";
  MM.CGIContents = "";
  dw.popupCommand("CGI Variable");
  if (MM.retVal == "OK")
  {
    var theResponse = MM.CGIContents;
    if (theResponse.length)
    {
      var siteURL = dw.getSiteRoot();
      if (siteURL.length)
      {
        dwscripts.addListValueToNote(siteURL, "CGI", theResponse);   
      }
      else
      {
...
function findDynamicSources()
{
  ...
  if (siteURL.length)
  {
    var bindingsArray = dwscripts.getListValuesFromNote(siteURL, "CGI");
    if (bindingsArray.length > 0)
    {
      retList.push(new DataSource(MM.LABEL_CGI, 
                                  CGI_FILENAME, 
                                  false, 
                                  "CGI.htm"))
    }
...
function generateDynamicDataRef(sourceName, bindingName, dropObject)
{
  ...
  var retStr = extPart.getInsertString("", "CGI_DataRef", paramObj);
...
function inspectDynamicDataRef(expression)
{
  ...
  {
    var params = extPart.findInString("CGI_DataRef", expression);
    if (params)
    {
