...
function formatDynamicDataRef(str, format)
{
	...
	if (iStart > -1)
	{
		...
		if (iEnd > -1)
		{
			...
			if (format.strNamedFormat && format.strNamedFormat.length) 
			{
				ret = str.substring(0, iStart+1) + format.func + "(" + str.substring(iStart+1, iEnd) + ",'" + format.strNamedFormat + "')" + str.substr(iEnd);
			}
			else
			{
