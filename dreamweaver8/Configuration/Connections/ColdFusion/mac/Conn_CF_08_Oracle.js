...
var HELP_DOC = MM.HELP_DSsettingOracle;
...
var AdvancedSettingsFileName = "../AdvancedSettings/Conn_CF_08_Oracle_Adv.htm";
...
function initializeUI() { 
	...
	SID_NAME_OBJ = dwscripts.findDOMObject("SIDName");
...
function inspectConnection(connParams) { 
	if (MM.DSNName) {
		...
		if (structure) {
			...
			SID_NAME_OBJ.value = getDataSourceParameter(CONN_NAME_OBJ.value, 'sid', PARAM_STRING, structure);
...
function getParams() {
	...
	// standard values
	if (SID_NAME_OBJ.value) {
		param.sid = SID_NAME_OBJ.value;
	}
...
function getParamsFromStructure(structure) {
	...
	if (structure) {
		if (getDataSourceParameter(CONN_NAME_OBJ.value, 'sid', PARAM_STRING, structure)) {
			result.sid = getDataSourceParameter(CONN_NAME_OBJ.value, 'sid', PARAM_STRING, structure);
		}
...
//
// DESCRIPTION:
//   This function creates a Oracle connection on the server
//
// ARGUMENTS:
...
function createDatasource(params) {
	...
	executeRequest(buildRequest('cfide.adminapi.datasource', 'setOracle', params, getAdminPassword()));
	return 1;
}
...
function isValidConnection() {
	...
	if (retVal) {
		if (SID_NAME_OBJ.value == "") {
			alert(MM.MSG_SIDNameRequired);
			document.theForm.SIDName.focus();
			return false;
		}
	}
