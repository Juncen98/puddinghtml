...
var HELP_DOC = MM.HELP_DSsettingInformix;

var CONN_NAME_OBJ;
var DB_NAME_OBJ;
var INFORMIX_SERVER_NAME_OBJ;
var HOST_NAME_OBJ;
var PORT_OBJ;
var USERNAME_OBJ;
var PASSWORD_OBJ;
...
var AdvancedSettingsFileName = "../AdvancedSettings/Conn_CF_02_Informix_Adv.htm";
...
function initializeUI() { 
	...
	INFORMIX_SERVER_NAME_OBJ = dwscripts.findDOMObject("InformixServerName");
...
function inspectConnection(connParams) {
	if (MM.DSNName) {
		...
		if (structure) {
			...
			INFORMIX_SERVER_NAME_OBJ.value = getDataSourceParameter(CONN_NAME_OBJ.value, 'InformixServer', PARAM_STRING, structure);
...
//   nothing
//--------------------------------------------------------------------
function inspectConnection_advanced(structure) {
...
function getParams() {
	...
	if (INFORMIX_SERVER_NAME_OBJ.value) {
		param.InformixServer = INFORMIX_SERVER_NAME_OBJ.value;
	}
...
function getParamsFromStructure(structure) {
	...
	if (structure) {
		...
		if (getDataSourceParameter(CONN_NAME_OBJ.value, 'InformixServer', PARAM_STRING, structure)) {
			result.InformixServer = getDataSourceParameter(CONN_NAME_OBJ.value, 'InformixServer', PARAM_STRING, structure);
		}
...
function createDatasource(params) {
	...
	executeRequest(buildRequest('cfide.adminapi.datasource', 'setInformix', params, getAdminPassword()));
	return 1;
}
...
function isValidConnection() {
	...
	if (retVal) {
		if (INFORMIX_SERVER_NAME_OBJ.value == "") {
			alert(MM.MSG_InformixServerNameRequired);
			document.theForm.InformixServerName.focus();
			return false;
		}
	}
